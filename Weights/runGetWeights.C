...
void runGetWeights()
{
...
#if !defined (__CINT__) || defined (__CLING__)
    gInterpreter->LoadMacro("AliAnalysisGetWeights.cxx++g");
    AliAnalysisGetWeights *task = reinterpret_cast<AliAnalysisGetWeights*>(gInterpreter->ExecuteMacro("AddGetWeightsTask.C(\"FlowTask\")"));
#else
    gROOT->LoadMacro("AliAnalysisGetWeights.cxx++g");
    gROOT->LoadMacro("AddGetWeightsTask.C");
    AliAnalysisGetWeights *task = AddGetWeightsTask();
#endif
    ...
    if(local) {
        ...
        alienHandler->SetAdditionalLibs("AliAnalysisGetWeights.cxx AliAnalysisGetWeights.h");
        // alienHandler->SetAdditionalLibs("PWGCFFLOWGF.par libPWGEMCALbase.so");
        alienHandler->SetAnalysisSource("AliAnalysisGetWeights.cxx");
        ...
        alienHandler->SetGridWorkingDir("Weights_working_dir");
        alienHandler->SetGridOutputDir("Weights_output_dir");
